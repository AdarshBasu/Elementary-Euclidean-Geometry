...
def IntPointAngle (a : Point) (A : Angle) : Prop :=
  let A1 := Angle.mk a A.Pivot A.p1
  let A2 := Angle.mk a A.Pivot A.p2
  (mAngle (A1) < mAngle  A) 
...
axiom mAngleAdd (a : Point) (A : Angle) (h : IntPointAngle a A) : 
...
axiom StraightAngle (a b c : Point) (_ : in_between a b c) : mAngle (Angle.mk a b c) = 180
...
theorem COAequal (l1 l2 l3 : Line) (h :  ¬ IsParallel l1 l3) (h' : IsParallel l1 l2 )
...
axiom RSL (a b c o: Point )(l : Line) (R : Segment)
(p1 : ¬ a = b)
(p2 : ¬ b = c)
(h1 : lies_on a (line_from_seg R))
(h2 : lies_on b l)
(h3 : lies_on c l)
(h4 : lies_on o l) 
(h5 : lies_on o (line_from_seg R)):

let BOA := Angle.mk b o a
let AOC := Angle.mk a o c
mAngle (BOA) + mAngle (AOC) = 180
...
have lem2 : lies_on a l1 := 
  And.left h1

have lem3 : lies_on b l1 :=
  And.right h1 

have lem4 : lies_on e l1 :=
  And.left h3

have lem5 : lies_on e (line_from_seg R) := by 
  apply And.right (contain_end_points R)  

have lem6 : lies_on a (line_from_seg R') :=
  And.left (contain_end_points R')  

have lem7 : lies_on d (line_from_seg R'') :=   
  And.left (contain_end_points R'')

have lem8 : lies_on e l2 :=
  And.right h3

have lem9 : lies_on e (line_from_seg R') :=
  And.right (contain_end_points R')

have lem10 : lies_on e (line_from_seg R'') :=
  And.right (contain_end_points R'')

have lem11 : lies_on d l2 :=
  And.right h2  

have lem12 : lies_on c l2 :=
  And.left h2  

have sumAEC_CEB : mAngle (AEC) + mAngle (CEB) = 180 := by
  apply RSL c a b e l1 R lem1 lem2 lem3 lem4 lem5 -- mAngle (AEC)+ mAngle (BEC) = 180

have equalAEC_CEA : AEC = Angle.mk c e a := by
  apply AngleSymm a e c

have equalAED_DEA : AED = Angle.mk d e a := by
 apply AngleSymm a e d

have sumAEC_AED : mAngle (AEC) + mAngle (AED) = 180 := by
 apply RSL a c d e l2 R' lem6 lem12 lem11 lem8 lem9



have sumAEC_AED : mAngle (CEA) + mAngle (AED) = 180 := by
 apply RSL a c d e l2 R' lem6 lem12 lem11 lem8 lem9
 -- subtract above two → mAngle (BEC) = mAngle (AED)

have sumAED_BED : mAngle (AED) + mAngle (DEB) = 180 := by
 apply RSL d a b e l1 R'' lem7 lem3 lem2 lem4 lem7   
-- subtract above two → mAngle (AEC) = mAngle (BED)

-/

--  It says that non parallel lines l1 and l2 intersect at p.
def IntersectFun (l1 l2 : Line) (_: ¬ IsParallel l1 l2 ) (p : Point):
  Prop := lies_on p l1 ∧ lies_on p l2 




-- equality of measure of "symmetric" angles
def SymmAngles (a o b : Point): Prop :=
  mAngle (Angle.mk a o b) = mAngle (Angle.mk b o a)


theorem VOAequal (a b c d o : Point) 
(h1 : ¬a = b) 
(h2 : ¬c = d)
(h3 : in_between a o b ∧ in_between c o d)
(h4 : ¬IsParallel (Line_of_two_points a b h1) (Line_of_two_points c d h2)) 
(h5: IntPointAngle c (Angle.mk a o b)) 
(h6: IntPointAngle a (Angle.mk c o d)):
let AOC := Angle.mk a o c
let BOD := Angle.mk b o d
mAngle (AOC) = mAngle (BOD)
:= by 
...
have lem1 : mAngle (AOB) = 180 := StraightAngle a o b (And.left h3)
have lem2 : mAngle (COD) = 180 := StraightAngle c o d (And.right h3)
...
have lem6: mAngle (AOC) = mAngle (BOD) by simp lem1 lem2 lem3 lem4
sorry
