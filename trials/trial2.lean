...
variable (geomi : IncidenceGeometry)
...
structure EuclidGeometry extends IncidenceGeometry where
  ...
  lies_on_segment (p : geomi.Point) (seg : Segment geomi) : geomi.in_between seg.p1 p seg.p2
...
structure SA where
  StraightAngle (a b c : geom.Point) (h : geom.in_between a b c) : 
...
:= by 
  ...
  let COB := Angle.mk c o b
  have lem1 : mAngle (AOB) = 180 := SA.StraightAngle a o b (And.left h3)
